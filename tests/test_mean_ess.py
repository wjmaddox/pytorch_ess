...
from pytorch_ess import MeanEllipticalSliceSampler
...
class TestMeanEllipticalSlice(unittest.TestCase):
    def test_m_ess(self, nsamples=10000):
        ...
        m_ess_runner = MeanEllipticalSliceSampler(init, prior_dist, lfn, nsamples)
        ...
        # print(est_mean)
        est_mean = torch.mean(samples, 1)
        # self.assertLess(est_mean.norm(), 0.1)

        est_cov = torch.tensor(np.cov(samples))

        mean_difference = (est_mean - true_postmu).norm()
        self.assertLess(mean_difference.norm(), 0.05)
        
        covariance_difference = (est_cov - true_postsigma).norm()
        self.assertLess(covariance_difference.norm(), 0.1)
